...
const commonLogEntryFormatters = {
    ...
    donate: entry => `${entry.subject} donated ${entry.donation} pre-tax gold to ${entry.target || entry.target_player}`,
    upgrade_range: entry => `${entry.subject} upgraded their range`,
    bounty: entry => `${entry.subject} placed a ${entry.bounty} gold bounty on ${entry.target || entry.target_player}`,
    stimulus: entry => `${entry.subject} granted a stimulus of 1 action to ${entry.target || entry.target_player}`,
    grant_life: entry => `${entry.subject} granted 1 life to ${entry.target || entry.target_player}`,
    ...
    smite: entry => `${entry.subject} smote ${entry.target || entry.target_player}`,
    heal: entry => `${entry.subject} healed ${entry.target || entry.target_player}`,
